...
public class EntityFieldNameConverterBak extends AbstractEntityFieldNameConverter{
    @Override
    IndexAbstractField getField(@Nullable String s, ConvertContext context) {
        if(isNotCheckElement(context)) return null;//如果不是需要检查的内容，则跳过

        Collection<IndexAbstractField> fields = getFields(context);
    ...
    @Override
    @NotNull Collection<IndexAbstractField> getFieldVariants(ConvertContext context) {
        return getFields(context);
    }

    private Collection<IndexAbstractField> getFields(ConvertContext context) {
        Collection<IndexAbstractField> result = new ArrayList<>();
                ...
                if(curKeyMap !=null) {
                    ...
                    switch (curAttributeName) {
                        case (KeyMap.ATTR_FIELD_NAME)->{
                            ...
                            result.addAll(EntityUtils.getAbstractFieldListFromViewEntityByAlias(
                                    curViewEntity,
                    ...
                    switch (curAttributeName) {
                        case ECondition.ATTR_FIELD_NAME -> {
                            if(entityAlias.isEmpty()) {
                                ...
                                result.addAll(EntityUtils.getViewEntityFieldList(curViewEntity)
            ...
            if(curAlias != null) {
                ...
                if(entityAlias.isEmpty()) {
                   ...
                   if(curComplexAliasField != null) {
                       result.addAll(EntityUtils.getAbstractFieldListFromViewEntityByAlias(curViewEntity,
