...
public class RelationshipReferenceConverter implements CustomReferenceConverter<Relationship> {
//    @Override
...
//
//    }
    private Optional<Relationship> getRelationship(String related, PsiElement psiElement) {
        List<Relationship> relationshipList = EntityUtils.getRelationshipListByRelationshipElement(psiElement);
        return relationshipList.stream().filter(
                item->EntityUtils.isThisRelationshipRelatedName(item,related)
        ).findFirst();
