...
//        //如果location中包含${,则表示是动态内容，无法处理
//        if(valueStr.contains("${")) return PsiReference.EMPTY_ARRAY;
//
...
//        return result.toArray(new PsiReference[0]);
    ...
    private PsiReference @NotNull [] processLocalFormExtend(GenericDomValue<String> value, PsiElement element, ConvertContext context) {
        String valueStr = value.getStringValue();
        if(valueStr ==null) return PsiReference.EMPTY_ARRAY;
